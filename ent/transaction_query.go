...
func (tq *TransactionQuery) QueryBooking() *BookingQuery {
...
func (tq *TransactionQuery) QueryParcel() *ParcelQuery {
...
func (tq *TransactionQuery) QueryCompany() *CompanyQuery {
...
func (tq *TransactionQuery) Clone() *TransactionQuery {
	...
	return &TransactionQuery{
		...
		withParcel:  tq.withParcel.Clone(),
...
func (tq *TransactionQuery) WithBooking(opts ...func(*BookingQuery)) *TransactionQuery {
	query := (&BookingClient{config: tq.config}).Query()
...
func (tq *TransactionQuery) WithParcel(opts ...func(*ParcelQuery)) *TransactionQuery {
	query := (&ParcelClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TransactionQuery) WithCompany(opts ...func(*CompanyQuery)) *TransactionQuery {
	query := (&CompanyClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if tq.withBooking != nil || tq.withParcel != nil || tq.withCompany != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withBooking; query != nil {
		if err := tq.loadBooking(ctx, query, nodes, nil,
			func(n *Transaction, e *Booking) { n.Edges.Booking = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withParcel; query != nil {
		if err := tq.loadParcel(ctx, query, nodes, nil,
			func(n *Transaction, e *Parcel) { n.Edges.Parcel = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withCompany; query != nil {
		if err := tq.loadCompany(ctx, query, nodes, nil,
			func(n *Transaction, e *Company) { n.Edges.Company = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TransactionQuery) loadBooking(ctx context.Context, query *BookingQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Booking)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].booking_transaction == nil {
			continue
		}
		fk := *nodes[i].booking_transaction
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "booking_transaction" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TransactionQuery) loadParcel(ctx context.Context, query *ParcelQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Parcel)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].parcel_transaction == nil {
			continue
		}
		fk := *nodes[i].parcel_transaction
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "parcel_transaction" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TransactionQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Company)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].company_transactions == nil {
			continue
		}
		fk := *nodes[i].company_transactions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "company_transactions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
