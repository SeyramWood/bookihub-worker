...
func (rsq *RouteStopQuery) QueryCompany() *CompanyQuery {
	query := (&CompanyClient{config: rsq.config}).Query()
...
func (rsq *RouteStopQuery) QueryTrip() *TripQuery {
	query := (&TripClient{config: rsq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rsq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rsq *RouteStopQuery) Clone() *RouteStopQuery {
	...
	return &RouteStopQuery{
		...
		withCompany: rsq.withCompany.Clone(),
		withTrip:    rsq.withTrip.Clone(),
...
func (rsq *RouteStopQuery) WithCompany(opts ...func(*CompanyQuery)) *RouteStopQuery {
	query := (&CompanyClient{config: rsq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rsq.withCompany = query
	return rsq
}
...
func (rsq *RouteStopQuery) WithTrip(opts ...func(*TripQuery)) *RouteStopQuery {
	query := (&TripClient{config: rsq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rsq *RouteStopQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RouteStop, error) {
	...
	if query := rsq.withCompany; query != nil {
		if err := rsq.loadCompany(ctx, query, nodes, nil,
			func(n *RouteStop, e *Company) { n.Edges.Company = e }); err != nil {
			return nil, err
		}
	}
	if query := rsq.withTrip; query != nil {
		if err := rsq.loadTrip(ctx, query, nodes,
			func(n *RouteStop) { n.Edges.Trip = []*Trip{} },
			func(n *RouteStop, e *Trip) { n.Edges.Trip = append(n.Edges.Trip, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rsq *RouteStopQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*RouteStop, init func(*RouteStop), assign func(*RouteStop, *Company)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*RouteStop)
	for i := range nodes {
		if nodes[i].company_stops == nil {
			continue
		}
		fk := *nodes[i].company_stops
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "company_stops" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rsq *RouteStopQuery) loadTrip(ctx context.Context, query *TripQuery, nodes []*RouteStop, init func(*RouteStop), assign func(*RouteStop, *Trip)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*RouteStop)
	nids := make(map[int]map[*RouteStop]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(routestop.TripTable)
		s.Join(joinT).On(s.C(trip.FieldID), joinT.C(routestop.TripPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(routestop.TripPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(routestop.TripPrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*RouteStop]struct{}{byID[outValue]: {}}
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "trip" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
