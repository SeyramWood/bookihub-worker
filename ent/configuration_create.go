...
func (cc *ConfigurationCreate) SetCharge(s *schema.Charge) *ConfigurationCreate {
	cc.mutation.SetCharge(s)
	return cc
}
...
func (cc *ConfigurationCreate) defaults() {
	...
	if _, ok := cc.mutation.Charge(); !ok {
		v := configuration.DefaultCharge
		cc.mutation.SetCharge(v)
	}
}
...
func (cc *ConfigurationCreate) check() error {
	...
	if _, ok := cc.mutation.Charge(); !ok {
		return &ValidationError{Name: "charge", err: errors.New(`ent: missing required field "Configuration.charge"`)}
	}
	return nil
}
...
func (cc *ConfigurationCreate) createSpec() (*Configuration, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.Charge(); ok {
