...
type NotificationQuery struct {
	...
	withBookibusUser *BookibusUserQuery
	withCompanyUser  *CompanyUserQuery
...
// QueryBookibusUser chains the current query on the "bookibus_user" edge.
func (nq *NotificationQuery) QueryBookibusUser() *BookibusUserQuery {
	query := (&BookibusUserClient{config: nq.config}).Query()
...
// QueryCompanyUser chains the current query on the "company_user" edge.
func (nq *NotificationQuery) QueryCompanyUser() *CompanyUserQuery {
	query := (&CompanyUserClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NotificationQuery) QueryCompany() *CompanyQuery {
	query := (&CompanyClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NotificationQuery) Clone() *NotificationQuery {
	...
	return &NotificationQuery{
		...
		withBookibusUser: nq.withBookibusUser.Clone(),
		withCompanyUser:  nq.withCompanyUser.Clone(),
		withCustomer:     nq.withCustomer.Clone(),
		withCompany:      nq.withCompany.Clone(),
...
// WithBookibusUser tells the query-builder to eager-load the nodes that are connected to
// the "bookibus_user" edge. The optional arguments are used to configure the query builder of the edge.
func (nq *NotificationQuery) WithBookibusUser(opts ...func(*BookibusUserQuery)) *NotificationQuery {
	query := (&BookibusUserClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withBookibusUser = query
	return nq
}

// WithCompanyUser tells the query-builder to eager-load the nodes that are connected to
// the "company_user" edge. The optional arguments are used to configure the query builder of the edge.
func (nq *NotificationQuery) WithCompanyUser(opts ...func(*CompanyUserQuery)) *NotificationQuery {
	query := (&CompanyUserClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withCompanyUser = query
	return nq
}
...
func (nq *NotificationQuery) WithCompany(opts ...func(*CompanyQuery)) *NotificationQuery {
	query := (&CompanyClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withCompany = query
	return nq
}
...
//		GroupBy(notification.FieldEvent).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Notification.Query().
//		Select(notification.FieldEvent).
//		Scan(ctx, &v)
...
func (nq *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	var (
		...
		_spec       = nq.querySpec()
		loadedTypes = [4]bool{
			nq.withBookibusUser != nil,
			nq.withCompanyUser != nil,
			nq.withCustomer != nil,
			nq.withCompany != nil,
		}
	)
	if nq.withCompany != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := nq.withBookibusUser; query != nil {
		if err := nq.loadBookibusUser(ctx, query, nodes,
			func(n *Notification) { n.Edges.BookibusUser = []*BookibusUser{} },
			func(n *Notification, e *BookibusUser) { n.Edges.BookibusUser = append(n.Edges.BookibusUser, e) }); err != nil {
			return nil, err
		}
	}
	if query := nq.withCompanyUser; query != nil {
		if err := nq.loadCompanyUser(ctx, query, nodes,
			func(n *Notification) { n.Edges.CompanyUser = []*CompanyUser{} },
			func(n *Notification, e *CompanyUser) { n.Edges.CompanyUser = append(n.Edges.CompanyUser, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := nq.withCompany; query != nil {
		if err := nq.loadCompany(ctx, query, nodes, nil,
			func(n *Notification, e *Company) { n.Edges.Company = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NotificationQuery) loadBookibusUser(ctx context.Context, query *BookibusUserQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *BookibusUser)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(notification.BookibusUserTable)
		s.Join(joinT).On(s.C(bookibususer.FieldID), joinT.C(notification.BookibusUserPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(notification.BookibusUserPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(notification.BookibusUserPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*BookibusUser](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "bookibus_user" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (nq *NotificationQuery) loadCompanyUser(ctx context.Context, query *CompanyUserQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *CompanyUser)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Notification)
	nids := make(map[int]map[*Notification]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(notification.CompanyUserTable)
		s.Join(joinT).On(s.C(companyuser.FieldID), joinT.C(notification.CompanyUserPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(notification.CompanyUserPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(notification.CompanyUserPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*CompanyUser](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "company_user" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (nq *NotificationQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Company)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Notification)
	for i := range nodes {
		if nodes[i].company_notifications == nil {
			continue
		}
		fk := *nodes[i].company_notifications
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "company_notifications" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
