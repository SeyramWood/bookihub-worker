...
func (pq *PayoutQuery) QueryTransaction() *TransactionQuery {
...
func (pq *PayoutQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (pq *PayoutQuery) FirstIDX(ctx context.Context) int {
...
func (pq *PayoutQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (pq *PayoutQuery) OnlyIDX(ctx context.Context) int {
...
func (pq *PayoutQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (pq *PayoutQuery) IDsX(ctx context.Context) []int {
	ids, err := pq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (pq *PayoutQuery) WithTransaction(opts ...func(*TransactionQuery)) *PayoutQuery {
	query := (&TransactionClient{config: pq.config}).Query()
...
//
//	client.Payout.Query().
//		GroupBy(payout.FieldAccountName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Payout.Query().
//		Select(payout.FieldAccountName).
//		Scan(ctx, &v)
...
func (pq *PayoutQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Payout, error) {
	...
	if query := pq.withTransaction; query != nil {
		if err := pq.loadTransaction(ctx, query, nodes, nil,
			func(n *Payout, e *Transaction) { n.Edges.Transaction = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PayoutQuery) loadTransaction(ctx context.Context, query *TransactionQuery, nodes []*Payout, init func(*Payout), assign func(*Payout, *Transaction)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Payout)
	for i := range nodes {
		if nodes[i].payout_transaction == nil {
			continue
		}
		fk := *nodes[i].payout_transaction
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "payout_transaction" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *PayoutQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(payout.Table, payout.Columns, sqlgraph.NewFieldSpec(payout.FieldID, field.TypeInt))
	_spec.From = pq.sql
...
func (pq *PayoutQuery) Modify(modifiers ...func(s *sql.Selector)) *PayoutSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (ps *PayoutSelect) Modify(modifiers ...func(s *sql.Selector)) *PayoutSelect {
