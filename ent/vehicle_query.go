...
func (vq *VehicleQuery) QueryCompany() *CompanyQuery {
...
func (vq *VehicleQuery) QueryImages() *VehicleImageQuery {
	query := (&VehicleImageClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VehicleQuery) QueryTrips() *TripQuery {
	query := (&TripClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VehicleQuery) Clone() *VehicleQuery {
	...
	return &VehicleQuery{
		...
		withCompany: vq.withCompany.Clone(),
		withImages:  vq.withImages.Clone(),
		withTrips:   vq.withTrips.Clone(),
...
func (vq *VehicleQuery) WithCompany(opts ...func(*CompanyQuery)) *VehicleQuery {
	query := (&CompanyClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withCompany = query
	return vq
}
...
func (vq *VehicleQuery) WithImages(opts ...func(*VehicleImageQuery)) *VehicleQuery {
	query := (&VehicleImageClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (vq *VehicleQuery) WithTrips(opts ...func(*TripQuery)) *VehicleQuery {
	query := (&TripClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (vq *VehicleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Vehicle, error) {
	...
	if query := vq.withCompany; query != nil {
		if err := vq.loadCompany(ctx, query, nodes, nil,
			func(n *Vehicle, e *Company) { n.Edges.Company = e }); err != nil {
			return nil, err
		}
	}
	if query := vq.withImages; query != nil {
		if err := vq.loadImages(ctx, query, nodes,
			func(n *Vehicle) { n.Edges.Images = []*VehicleImage{} },
			func(n *Vehicle, e *VehicleImage) { n.Edges.Images = append(n.Edges.Images, e) }); err != nil {
			return nil, err
		}
	}
	if query := vq.withTrips; query != nil {
		if err := vq.loadTrips(ctx, query, nodes,
			func(n *Vehicle) { n.Edges.Trips = []*Trip{} },
			func(n *Vehicle, e *Trip) { n.Edges.Trips = append(n.Edges.Trips, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VehicleQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*Vehicle, init func(*Vehicle), assign func(*Vehicle, *Company)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Vehicle)
	for i := range nodes {
		if nodes[i].company_vehicles == nil {
			continue
		}
		fk := *nodes[i].company_vehicles
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "company_vehicles" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (vq *VehicleQuery) loadImages(ctx context.Context, query *VehicleImageQuery, nodes []*Vehicle, init func(*Vehicle), assign func(*Vehicle, *VehicleImage)) error {
	...
	query.withFKs = true
	query.Where(predicate.VehicleImage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(vehicle.ImagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.vehicle_images
		if fk == nil {
			return fmt.Errorf(`foreign-key "vehicle_images" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "vehicle_images" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (vq *VehicleQuery) loadTrips(ctx context.Context, query *TripQuery, nodes []*Vehicle, init func(*Vehicle), assign func(*Vehicle, *Trip)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Vehicle)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Trip(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(vehicle.TripsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.vehicle_trips
		if fk == nil {
			return fmt.Errorf(`foreign-key "vehicle_trips" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "vehicle_trips" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (vq *VehicleQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(vehicle.Table, vehicle.Columns, sqlgraph.NewFieldSpec(vehicle.FieldID, field.TypeInt))
	_spec.From = vq.sql
